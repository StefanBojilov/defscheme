...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Modified for femmtolisp by Stefan Bojilov 2016-10-5
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;



;;;;;;;;;;;;;;;;;;;;;;;;; prlude - adapt to scheme implemntation ;;;;;;;;;;;;;

;;; chibi needs these
;;; (import (chibi ast) (chibi time) (scheme cxr) (srfi 33) (srfi 39)  (scheme inexact)  (scheme r5rs) (scheme small)  (chibi))

(load "aliases.scm")
;;; (define math.pi (atan 0 -1))     ;;; different implementations have it differently
;;; (define math.pi (* (atan 1) 4))
(define math.pi   3.141592653589793)
(define math.pi2  (/ math.pi 2))
(define cos math.cos)
(define sin math.sin)
;;; (define (sin x) (cos (- math.pi2 x)))
(define (expt base power) (let recur ((res 1) (pow power))
     (if (> pow 0)
         (recur (* base res) (- pow 1))
         res)))
...
;;;   (eval (macro-expand '(ppp nv2)))
;;;   (eval (macro-expand '(ppp j)))  (eval (macro-expand '(ppp i)))
    ...
    (do ((l 1 (+ l 1)))                 ;loop thru stages (syntax converted
        ...
        (set! ur 1.0)
        (set! ui 0.)
        (set! wr (cos (/ math.pi le1)))
        (set! wi (sin (/ math.pi le1)))
...
;;;   (eval (macro-expand '(ppp ip))) (eval (macro-expand '(ppp i)))
